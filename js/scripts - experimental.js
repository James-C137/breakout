...
function hitDetection() {
    for (x = 0; x < numCols; x++) {
        for (y = 0; y < numRows; y++) {
            ...
            if (brickArray[x][y] === 1 && velocityX < 0 && ((ballX >= (brickX * x) + brickMarginX + (brickW / 2) && ballX <= (brickX * x) + brickMarginX + brickW + ballRadius) && (ballY >= (brickY * y) + brickMarginY && ballY <= (brickY * y) + brickMarginY + brickH * 1.5))) {
                velocityX = -velocityX
                brickArray[x][y] = 0;
                score++;
                numBricks--;
...
function reset() {
    ...
    if (lives === 0) {
        finalScore = finalScore * (20000 / timer);
